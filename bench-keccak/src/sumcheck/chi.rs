...
pub fn prove_sumcheck_chi(
    ...
    for _ in 0..size {
        ...
        for i in 0..e0.len() {
            for j in 0..pi.len() {
                // Evaluation at 0
                p0 += e0[i] * beta[j] * xor(pi[j].0[i], (Fr::one() - pi[add_col(j, 1)].0[i]) * pi[add_col(j, 2)].0[i]);

                // Evaluation at -1
                let eem1 = e0[i] + e0[i] - e1[i];
                // TODO: these values can be calculated once
                let piem1 = pi[j].0[i] + pi[j].0[i] - pi[j].1[i];
                let piem1_5 = pi[add_col(j, 1)].0[i] + pi[add_col(j, 1)].0[i] - pi[add_col(j, 1)].1[i];
                let piem1_10 = pi[add_col(j, 2)].0[i] + pi[add_col(j, 2)].0[i] - pi[add_col(j, 2)].1[i];
                pem1 += eem1 * beta[j] * xor(piem1, (Fr::one() - piem1_5) * piem1_10);

                // Evaluation at ∞
                let beta_m2 = -beta[j] - beta[j];
                p4 += beta_m2 * (e1[i] - e0[i]) * (pi[j].1[i] - pi[j].0[i]) * (pi[add_col(j, 1)].0[i] - pi[add_col(j, 1)].1[i]) * (pi[add_col(j, 2)].1[i] - pi[add_col(j, 2)].0[i]);

                // Evaluation at 2
                let ee2 = e1[i] + e1[i] - e0[i];
                let pie2 = pi[j].1[i] + pi[j].1[i] - pi[j].0[i];
                let pie2_5 = pi[add_col(j, 1)].1[i] + pi[add_col(j, 1)].1[i] - pi[add_col(j, 1)].0[i];
                let pie2_10 = pi[add_col(j, 2)].1[i] + pi[add_col(j, 2)].1[i] - pi[add_col(j, 2)].0[i];
    ...
    for j in 0..pis.len() {
        checksum += beta[j] * xor(pis[j][0],(Fr::one() - pis[add_col(j, 1)][0]) * (pis[add_col(j, 2)][0]))
    }
    assert_eq!(sum, e[0] * checksum);
    (sum, rs)
}
