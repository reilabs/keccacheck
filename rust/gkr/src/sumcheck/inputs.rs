...
pub fn prove_inputs(
    ...
    let ((mut eq1, mut eq2), (mut eq3, mut a)) = rayon::join(
        || {
            rayon::join(
        ...
        || {
            rayon::join(
                || calculate_evaluations_over_boolean_hypercube_for_eq(r3),
                || {
    ...
    prove_sumcheck_inputs(
        transcript, num_vars, beta1, beta2, beta3, &mut eq1, &mut eq2, &mut eq3, &mut a, sum,
    )
}

#[instrument(skip_all)]
pub fn prove_sumcheck_inputs(
    transcript: &mut Prover,
    size: usize,
    beta_a: &[Fr],
    beta_b: &[Fr],
    ...
    for _ in 0..size {
        ...
        let (ec0, ec1) = eq_c.split_at(eq_c.len() / 2);
        let a = aij
            .iter()
            .map(|x| {
        ...
        let (p0t, p2t) = a
            ...
            .map(|(j, a)| {
                ...
                (
                    ba * p0_a + bb * p0_b + bc * p0_c,
                    ba * p2_a + bb * p2_b + bc * p2_c,
                )
            })
        ...
        // Update eq polynomials and a polynomials
        let len = eq_a.len();
        ((eq_a, eq_b), (eq_c, _)) = rayon::join(
    ...
    #[cfg(debug_assertions)]
    {
        ...
        for j in 0..aij.len() {
            checksum +=
                (beta_a[j] * eq_a[0] + beta_b[j] * eq_b[0] + beta_c[j] * eq_c[0]) * subclaims[j];
        }
...
mod test {
    ...
    fn inputs_no_recursion() {
        ...
        let alpha_c = (0..num_vars).map(|_| prover.read()).collect::<Vec<_>>();

        let beta_a = (0..STATE).map(|_| prover.read()).collect::<Vec<_>>();
        let beta_b = (0..STATE).map(|_| prover.read()).collect::<Vec<_>>();
        let beta_c = (0..STATE).map(|_| prover.read()).collect::<Vec<_>>();
        ...
        let real_sum: Fr = result
            ...
            .enumerate()
            .map(|(i, poly)| {
                beta_a[i] * eval_mle(poly, &alpha_a)
                    + beta_b[i] * eval_mle(poly, &alpha_b)
                    + beta_c[i] * eval_mle(poly, &alpha_c)
            })
            .sum();
