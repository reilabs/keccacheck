...
type KeccacheckWhirCircuit struct {
	Input  []frontend.Variable `gnark:",secret"`
	Output []frontend.Variable `gnark:",public"`
}

func NewKeccacheckWhirCircuit() *KeccacheckWhirCircuit {
	return &KeccacheckWhirCircuit{
...
func (circuit *KeccacheckWhirCircuit) Define(api frontend.API) error {
...
func Profile() {
	...
	p := profile.Start()
	_, _ = frontend.Compile(ecc.BN254.ScalarField(), r1cs.NewBuilder, NewKeccacheckWhirCircuit())
	p.Stop()
